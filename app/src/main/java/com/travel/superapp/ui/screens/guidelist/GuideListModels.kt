...
/** 按好评时：价格高 / 价格低（先按评分降序，再按价格） */
enum class GuideSortGoodRating {
    PriceHigh,
    PriceLow,
}
...
fun List<Guide>.filteredAndSorted(
    ...
    sortGoodRating: GuideSortGoodRating,
): List<Guide> {
    val base = when (mode) {
    ...
    return when (mode) {
        GuideFindMode.ByGoodRating -> when (sortGoodRating) {
            GuideSortGoodRating.PriceHigh -> base.sortedWith(
                compareByDescending<Guide> { it.rating }
                    .thenByDescending { it.priceYuanPerDay }
                    .thenBy { it.id },
            )
            GuideSortGoodRating.PriceLow -> base.sortedWith(
                compareByDescending<Guide> { it.rating }

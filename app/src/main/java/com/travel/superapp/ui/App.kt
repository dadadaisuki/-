...
import com.travel.superapp.ui.screens.WalkModuleScreen
...
private fun AppNavHost(
    ...
    NavHost(
        ...
        composable("entry/walk") { WalkModuleScreen(onBack = { navController.popBackStack() }) }
        composable("entry/bike") { BikeModuleScreen(onBack = { navController.popBackStack() }) }
        composable("entry/car") { CarTourModuleScreen(onBack = { navController.popBackStack() }) }
        composable("entry/group") { GroupModuleScreen(onBack = { navController.popBackStack() }) }

        AppRoute.entryRoutes.forEach { route ->
            if (route in listOf("entry/walk", "entry/bike", "entry/car", "entry/group")) return@forEach
